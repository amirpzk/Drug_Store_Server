...
import com.Prana.Latifi.exception.TokenNotValidateWithThisSecretPass;
...
public class JwtFactoryImp implements JwtFactory {
    ...
    public String getUsername(String token) {
        return getClaimFromToken(token.replace(env.getProperty("token.prefix"), ""), Claims::getSubject);
    }
    ...
    public UserRoleModel parseToken(String token) {
        ...
        String changedToken = token.substring(7);
        if (!this.validate(changedToken, env.getProperty("jwt.server.secret")))
            throw new TokenNotValidateWithThisSecretPass();
        String username = ((LinkedHashMap) Jwts.parser()
                .setSigningKey(env.getProperty("jwt.server.secret"))
                .parseClaimsJws(changedToken)
